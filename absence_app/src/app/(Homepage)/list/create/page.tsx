...
export default function Page() {
    ...
    async function handleSubmit(values: z.infer<typeof RegisterSchema>) {
        ...
        try {
            const response = await fetch("http://localhost:3000/absent/create", {
                method: "POST",
                headers: {
            ...
            if (response.ok) {
                toast.success("Employee's Absent registration Successful")
                router.push("/employee")
                return;
            } else {
                toast.error("Employee's Absent registration failed")
            }
        } catch (error) {
            toast.error("Employee's Absent registration failed")
        }
    ...
    return (
        <div className="w-full h-full min-h-screen bg-white flex flex-col gap-y-4 items-center justify-center">
            <h1 className="font-bold text-3xl">Create a Employee's List Account</h1>
            <Form {...form}>
